...
**Dealerbase look and feel.** Signing in lands on the Dealerbase home page (`/desk/dealer-home`). It works as the navigation hub for the whole app: tiles for vehicles, acquisitions, inspections, market values, leads, sales, expenses, the Inventory Assistant and document import, plus setup links and a "jump to" search. Each user sees only what their role can open. Below that is an at-a-glance overview: KPIs, the inventory pipeline, recent vehicles, follow-ups, sales trend and lot aging. An empty site also gets a getting-started checklist. The app also brands the desk (accent colour, logo, favicon, app name) and the sign-in page. Branding is applied on install, after each migrate and when the setup wizard finishes, and only where a setting still has its stock value, so a logo, app name or home page you set yourself is kept. Theme files live in `apps/dealer_management/dealer_management/public/css/`.

**AI token budget.** Every Claude request made by the Inventory Assistant and document import is logged in AI Usage Log. Set a monthly token budget in AI Settings (0 means no budget) and choose whether AI features stop once it is used up. The Inventory Assistant shows the month's usage against the budget, the current chat's tokens and Anthropic's per-minute rate-limit headroom, and you can also ask it about the budget in the chat. Anthropic's API does not report an account's credit balance, so the budget is kept by the app. The account's balance and spend limit are on the Claude Console billing page.

**Sidebar.** Dealerbase ships its own left-hand menu (the `Dealer Management` workspace sidebar) with links to vehicles, acquisitions, leads, sales, expenses, the Inventory Assistant, setup lists and admin settings. Without it, Dealerbase pages could end up showing ERPNext's Stock menu (Stock Entry, Purchase Receipt, Delivery Note, Material Request), which Dealerbase does not use. The menu's header reads Dealerbase (with the company name underneath) rather than a module name, and Dealerbase is set as the site's default app when none is chosen (System Settings → Default App), so signing in opens the Dealerbase home instead of the desktop of app icons.

**Workspaces.** Dealerbase hides the ERPNext workspaces and app icons it does not use (Accounting, Stock, Selling, Manufacturing, CRM and the rest). Regular users see only Dealerbase and cannot open the others, even by URL. System Managers also keep Frappe's admin tools (Users, System, Email, Data import, Printing, Integrations, Automation, Build), and can still reach the hidden workspaces from the desktop's hidden icons. This is re-applied after every migrate, because migrations restore the stock workspaces. To show everything again, run `bench --site <site> set-config dealerbase_show_all_workspaces 1` in a `railway ssh` session, then `bench --site <site> migrate`. That undoes exactly what was changed, and setting it back to 0 hides them again.
